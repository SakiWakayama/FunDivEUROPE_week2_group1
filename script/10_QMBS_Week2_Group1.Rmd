...
```{r}
#prepare data subset
spanish_plots <- subset(FunDivEurope, country == "Spain")
...
```{r}
FunDivEUROPE <- read_csv("../Data/FunDivEUROPE.csv")
Fun <- FunDivEUROPE[, c("country","stability.biomass.production","target.species.richness","age.distribution")]
FunFilter <- Fun %>% filter(country == "Romania")
```
...
```
```{r}
FunFilter$age.distribution <- factor(FunFilter$age.distribution) # age as factor
```
```{r}
Funancova <- lm(
  stability.biomass.production ~ 
    target.species.richness * age.distribution,
  data = FunFilter
)
```
```{r}
...
summary(Funancova)
```
The effect of tree species richness on stability does not differ between even- and uneven-aged forests.
...
significant effect of species richness on stability of biomass production (F₁,₂₄ = 7.61, p = 0.011)
The interaction between species richness and age was not significant (F₁,₂₄ = 0.14, p = 0.71) so the richness–stability relationship did not differ between even- and uneven-aged forests

```{r}
plot.Fun <- ggplot(FunFilter, aes(y = stability.biomass.production, x = target.species.richness))+
  geom_smooth(method = lm,  aes(col = age.distribution)) +
  geom_point(
    shape = 21,
    size = 3,
    alpha = 0.75,
  ...
  xlab("target.species.richness") +
  ylab("stability.biomass.production") +
  theme_bw()
